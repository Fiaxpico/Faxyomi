...
class KavitaApi(private val client: OkHttpClient, interceptor: KavitaInterceptor) {
    ...
    fun getNewToken(apiUrl: String, apiKey: String): String? {
        val request = POST(
            "$apiUrl/Plugin/authenticate?apiKey=$apiKey&pluginName=faxyomi-Kavita",
    ...
    private fun getLatestChapterRead(url: String): Double {
        ...
        val requestUrl = "${getApiFromUrl(url)}/faxyomi/latest-chapter?seriesId=$seriesId"
        try {
            with(json) {
    ...
    suspend fun updateProgress(track: Track): Track {
        val requestUrl = "${getApiFromUrl(
            track.tracking_url,
        )}/faxyomi/mark-chapter-until-as-read?seriesId=${getIdFromUrl(
            track.tracking_url,
